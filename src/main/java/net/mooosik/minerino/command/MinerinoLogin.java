                ...
                .then(argument("oauth", StringArgumentType.string())
                    .executes(context -> {
                       ...
                       config.setOauthKey(StringArgumentType.getString(context, "oauth"));
                        ...
                        config.save();
                        ((FabricClientCommandSource) context.getSource()).sendFeedback(new LiteralText("Successfully registered username " + config.getUsername()));
    ...
    private static int login(CommandContext context, ModConfig config) {
        if(config.getUsername().equals("") || config.getOauthKey().equals("")) {
            ((FabricClientCommandSource) context.getSource())
                    .sendFeedback(new LiteralText("Please sign in with an oAuthentication token first." +
        ...
        if(Twitch.setup()) {
            ((FabricClientCommandSource) context.getSource()).sendFeedback(new LiteralText("Successfully signed in as " + config.getUsername()));

            if(config.getChannels().size() > 0) {

                for (String channel: config.getChannels()
                ) {
                    Twitch.getClient().getChat().joinChannel(channel);
                    ((FabricClientCommandSource) context.getSource()).sendFeedback(new LiteralText("Connected to channel " + channel));
                }

            } else {
                ((FabricClientCommandSource) context.getSource()).sendFeedback(new LiteralText("No channels found to connect to. Use /minerino join <channel>"));

            }
            return 1;
        } else {
            ((FabricClientCommandSource) context.getSource()).sendFeedback(new LiteralText("Something went wrong!"));
            return -1;
        }
